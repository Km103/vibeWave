...
const SongsHistory = () => {
    ...
    useEffect(() => {
        if (activeSong?.name) {
            ...
            const parsedSongHistory = storedSongHistory
                ? JSON.parse(storedSongHistory)
                : [];
            if (
                parsedSongHistory?.find((song) => song?.id === activeSong?.id)
            ) {
                const updatedHistory = parsedSongHistory.filter(
